...
func Compare(i1, i2 interface{}, options ...CompareOptions) (errors []error) {
	...
	if len(options) == 1 {
		optionErrors = options[1].options.CheckFieldsFormation()
		errors = compareValues(reflect.ValueOf(i1), reflect.ValueOf(i2), options[1].options)
	} else {
		errors = compareValues(reflect.ValueOf(i1), reflect.ValueOf(i2), nil)
	}
	if errors != nil {
...
func compareStrings(v1, v2 reflect.Value, options compareOptions) []error {
	if v1.String() != v2.String() {
		return []error{fmt.Errorf("expected \"%s\" got \"%s\"", v1.String(), v2.String())}
	}
	return nil
}
...
func compareMaps(v1, v2 reflect.Value, options compareOptions) (errors []error) {
	...
	foundTupleFromKey := func(key reflect.Value) *keyValueTuple {
		for i, tuple := range keyValueTuples {
			if compareValues(tuple.key, key, nil) == nil { // TODO implements options
	...
	for iter.Next() {
		...
		if tuple != nil {
			errs := compareValues(tuple.value, iter.Value(), nil) // TODO implements options
			if len(errs) > 0 {
				// TODO there is maybe a problem with the key printing
				errs[len(errs)-1] = fmt.Errorf("error in key %v: %v", tuple.key.String(), errs[len(errs)-1])
				errors = append(errors, errs...)
			}
		} else if !nullValue(iter.Value()) {
			errors = append(errors, fmt.Errorf("(_, `%v')", iter.Key()))
		}
	}
	for _, tuple := range keyValueTuples {
		if !nullValue(tuple.value) {
			errors = append(errors, fmt.Errorf("(`%v', _)", tuple.key))
		}
	}
	if len(errors) > 0 {
		errors = append(errors, fmt.Errorf("there is %d errors", len(errors)))
...
func compareStructs(v1, v2 reflect.Value, options compareOptions) (errors []error) {
	...
	for i := 0; i < t2.NumField(); i++ {
		...
		if findField(fieldName) {
			errs := compareValues(v1.FieldByName(fieldName), v2.FieldByName(fieldName), nil) // TODO implements options
			if len(errs) > 0 {
			...
			errors = append(errors, fmt.Errorf("(_, `%v')", fieldName))
			errorFields = append(errorFields, fieldName)
		}
	}
	...
	for fieldName := range fieldNames {
		if !nullValue(v1.FieldByName(fieldName)) {
			errors = append(errors, fmt.Errorf("(`%v', _)", fieldName))
			errorFields = append(errorFields, fieldName)
		}
	}

	if len(errorFields) > 0 {
		errors = append(errors, fmt.Errorf("struct field [%s] differs", strings.Join(errorFields, ", ")))
	}
